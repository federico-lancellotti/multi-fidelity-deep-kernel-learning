...
from src.utils import plot_latent_dims, plot_error, len_of_episode
...
def test():
    ...
    ID = args["ID"]
    episodes = args["num_episodes_test"][1]
    T_start = 0
    T_test = len_of_episode(env_name=env_name) 
    L_test = T_test
    ...
    plot_latent_dims(z.detach().numpy(), var.detach().numpy(), T_start=T_start, T=L_test, dt=1, dims=ID, episodes=episodes, show=False, filename=filepath+"latent_space/")

    # Plot of reconstructions, predictions and extrapolation
    for index in range(episodes):
        print("Exporting reconstructions for episode ", index+1, "...")
        
        filepath_ep = filepath + "ep_" + str(index+1) + "/"
        if not os.path.exists(filepath_ep):
            os.makedirs(filepath_ep)

        # Plot of the reconstruction
        start = index * L_test
        end = start + L_test - 2
        
        mse, mse_next = reconstruct(model, input_data, mu_x, mu_next, z_fwd_LF, L_test, T_start, 1, start, end, index+1, filepath_ep)
        
        ## Plot the error
        plot_error([mse, mse_next], T_start, 1, ["MSE", "MSE_next"], filepath_ep)

        # Extrapolation
        N_iter = 50
        mse_extra = extrapolate(model, input_data, mu_next[start], z_fwd_LF, L_test, T_start, 1, filepath_ep, N_iter=N_iter, start=start)

        ## Plot the extrapolation error
        plot_error([mse_extra], T_start, 1, ["MSE (extrapolation)"], filepath_ep + "extrapolation/")
